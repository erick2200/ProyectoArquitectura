...
bool DireccionAcelerometro::abierto = true;
bool DireccionAcelerometro::izquierda = false ;
bool DireccionAcelerometro::derecha = true;
...
namespace cp{
	...
	void PlayerCar::handle_input(std::vector<bool> mask, float dt) {
		...
		if (DireccionAcelerometro::izquierda)e_position.x -= dx;
		else if (DireccionAcelerometro::derecha)e_position.x += dx;
		e_position += e_speed;
